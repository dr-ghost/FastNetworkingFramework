...
void netdev_init()
{
    ...
    netdev = netdev_alloc("172.31.22.34", "14:5a:fc:4f:0d:17", 1500);
}
...
int netdev_transmit(struct sk_buff *skb, uint8_t *dst_hw, uint16_t ethertype)
{
    ...
    ret = socket_write((char *)skb->data, skb->len);
...
void *netdev_rx_loop()
{
    while (running)
    {
        ...
        if (socket_read((char *)skb->data, BUFLEN) < 0)
        {
            perror("ERR: Read from socket_fd");
            free_skb(skb);
            return NULL;
        }
