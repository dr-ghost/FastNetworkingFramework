...
static inline void skb_queue_add(struct sk_buff_head *list, struct sk_buff *new_, struct sk_buff *next)
{
    list_add_tail(&new_->list, &next->list);
    list->qlen += 1;
}

static inline void skb_queue_tail(struct sk_buff_head *list, struct sk_buff *new_)
{
    list_add_tail(&new_->list, &list->head);
    list->qlen += 1;
}
